...
const {getInst, setInst} = require('./model/db.js');
...
async function initServer() {
  ...
  setInst(new db.Database());
  await require('../test/data/loader.js').loadIntoDB(getInst());
  ...
  app.use(asyncHan(async (req, res, next) => {
    ...
    if (uid !== undefined) {
      ...
      await getInst().doTransaction((t) => u.reload(t));
      req.user = u;
    } else {
      req.user = undefined;
    }
