...
class SQLDatabaseTransaction extends DatabaseTransaction {
  ...
  /**
   * Search all project IDs in the database, if any
   * @return {Number[]} the project IDs.
   */
  async findAllProjects() {
    await this.checkValid();
    const qstr = 'SELECT projID FROM Project';
    const res = (await this._query(qstr)).result.map((req) => rec.projID);
    return res;
  }

  /**
   * Search all teams in the database, if any
   * @return {Number[]} the IDs of all teams
   */
  async findAllTeams() {
    await this.checkValid();
    const qstr = 'SELECT tid FROM Team';
    const res = (await this._query(qstr)).result.map((rec) => rec.tid);
    return res;
  }
