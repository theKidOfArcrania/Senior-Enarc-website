...
class SQLDatabase extends Database {
   ...
   * @param {Object} userInfo the attributes of the user
   */
  async insertTeamInfo(uid, userInfo) {
    const createFacultyOrTeam = 'INSERT INTO Faculty_Or_Team (TeamID, ' +
      'is_reg_team) VALUES (? ,?)';
    await this.pcon.query(createFacultyOrTeam, [uid, false]);
    const $createTeam = 'INSERT INTO Employee (Tid, Assigned_proj, Budget,' +
      'Leader) VALUES(?, ?, ?, ?)';
    await this.pcon.query($createTeam, [uid, userInfo.assignedProj,
      userInfo.budget, userInfo.leader]);
  }
