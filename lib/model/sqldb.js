...
class SQLDatabase extends Database {
   ...
   * @param {String} uid      the user id to insert at.
   * @param {Object} userInfo the attributes of the user
   * @param {Array} skills    the skills of the student
   */
  async insertStudentInfo(uid, userInfo, skills) {
    ...
    await this.pcon.query(createSkills, skills);
  }
   ...
   * @param {Array}  skills      array of skills needed by the project
   */
  async insertProjectInfo(projID, projInfo, skills) {
  ...
  async loadProjectInfo(projID) {
    ...
    const $selectSkills = 'SELECT * FROM SkillsReq WHERE pid = ?';
    const res = (await this.pcon.query($selectProj, projID)).result;
    res.skillsReq = (await this.pcon.query($selectSkills, projID));
  ...
  async loadStudentInfo(uid) {
    const $selectStudent = 'SELECT * FROM Student, Skills WHERE suid = ?';
    const $selectSkill = 'SELECT * FROM Skills WHERE stuUID = ?';
    const res = (await this.pcon.query($selectStudent, uid)).result;
    res.skills = (await this.pcon.query($selectSkill, uid)).result;
    if (res.length != 1) throw new Error('No match with given uid');
    delete res[0].suid;
    return res[0];
  }
