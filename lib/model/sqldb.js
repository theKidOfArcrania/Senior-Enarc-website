...
class SQLDatabase extends Database {
  ...
  async insertFacultyInfo(uid, userInfo) {
    ...
    const createChoice = 'INSERT INTO Choice (tid, ranking, pid) ' +
      'VALUES(?, ?, ?)';
    for (const i of util.range(6)) {
      await this.pcon.query(createChoice, [userInfo.tid, i, null]);
    }
  }
  ...
  async insertTeamInfo(tid, teamInfo) {
    ...
    const $createTeam = 'INSERT INTO Team (tid, assignedProj, budget,' +
      'leader, password) VALUES(?, ?, IFNULL(?, 0), ?, ?)';
    await this.pcon.query($createTeam, [tid, teamInfo.assignedProj,
      teamInfo.budget, teamInfo.leader, teamInfo.password]);
    const createChoice = 'INSERT INTO Choice (tid, ranking, pid) ' +
      'VALUES(?, ?, ?)';
    const choices = teamInfo.choices || [];
    for (const i of util.range(6)) {
      await this.pcon.query(createChoice, [tid, i, choices[i]]);
    }
  }
  ...
  async loadTeamInfo(uid) {
    ...
    res[0].choices = await this.findTeamChoices(uid);
    return res[0];
  }
  /**
