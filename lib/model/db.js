...
class ProjectStatus {
   ...
   * @param {Boolean} visible    whether if a project with this status should be
   *                             visible
   * @param {Boolean} modifiable whether if a sponsor can still modify the
   *                             project proposal
   */
  constructor(name, visible, modifiable) {
    this.name = name;
   ...
   * @return {ProjectStatus} the project status enum value (or undefined)
   */
  static ofString(name) {
    return statusValues[name];
  }
}

// A project is recently submitted and needs to be reviewed by admin
ProjectStatus.SUBMITTED = new ProjectStatus('submitted', false, true);
// Project requires some revision to be made by the sponsor
ProjectStatus.NEEDS_REVISION = new ProjectStatus('needs-revision', false, true);
// Project is accepted and opened up for all students to view
ProjectStatus.ACCEPTED = new ProjectStatus('accepted', true, false);
// Project proposal is rejected.
ProjectStatus.REJECTED = new ProjectStatus('rejected', false, false);
// Project proposal is archived (i.e. not open for this year).
ProjectStatus.ARCHIVED = new ProjectStatus('archived', false, false);
Object.assign(statusValues, Object.values(ProjectStatus).map((val) => {
  if (val instanceof ProjectStatus) statusValues[val.name] = val;
}));
...
class DatabaseTransaction extends Object {
  ...
  async searchUserByEmail(email) {
    await this.checkValid();
    for (let uid of Object.keys(this._db.USER)) {
      uid = parseInt(uid);
  ...
  async searchTeamByName(name) {
    await this.checkValid();
    for (let tid of Object.keys(this._db.TEAM)) {
      tid = parseInt(tid);
      if (this._db.TEAM[tid].name === name) {
        return tid;
      }
    }
    return null;
    ...
    if (id in this._db[tableName]) {
      ...
      try {
          ...
          for (const [k, v] of Object.entries(this._db[tbl])) {
            if (v[fkey] === id) {
              if (dmode === null) { // ON DELETE SET NULL
                ...
                this._deleteEntity(tbl, k);
              } else { // ERROR
                throw new DBError('Foreign key constraint failure on ' +
                  `\`${tbl}\`.\`${fkey}\``);
              }
            }
          }
        ...
        if (tableName === 'FACULTY') {
          this._deleteEntity('FACULTY_OR_TEAM', old.tid);
        }
      } catch (e) {
        this.popSP();
        throw e;
  ...
  /**
   * Delete Project
   * @param {Integer} projID     the projID to delete.
   * @return {Boolean} true if something changed
   */
  async deleteProject(projID) {
    await this.checkValid();
    return await this._deleteEntity('PROJECT', projID);
  }
  ...
  async deleteUser(uid) {
    await this.checkValid();
    return await this._deleteEntity('USER', uid);
  }
  ...
  async deleteUTD(uid) {
    await this.checkValid();
    return await this._deleteEntity('UTD_PERSONNEL', uid);
  }
  ...
  async deleteStudent(uid) {
    await this.checkValid();
    return await this._deleteEntity('STUDENT', uid);
  }
  ...
  async deleteFaculty(uid) {
    ...
    await this._deleteEntity('FACULTY', uid);
  }
  ...
  async deleteEmployee(uid) {
    await this.checkValid();
    return await this._deleteEntity('EMPLOYEE', uid);
  }

  /**
   * Delete team from the database
   * @param {Integer} tid      the team id to delete
   * @return {Boolean} true if something changed
   */
  async deleteTeam(tid) {
    await this.checkValid();
    return await this._deleteEntity('FACULTY_OR_TEAM', tid);
  }
  ...
  async deleteCompany(cName) {
    await this.checkValid();
    return await this._insertEntity('COMPANY', 'name', cName);
  }
   ...
   * @return {Boolean} true if something changed
   */
  async deleteHelpTicket(hid) {
    await this.checkValid();
    return await this._deleteEntity('HELP_TICKET', hid);
  }

  /**
   * Delets an invite information
   * @param {Integer} inviteID     the invite id
   * @return {Boolean} true if something changed
   */
  async deleteInvite(inviteID) {
    await this.checkValid();
    return await this._deleteEntity('INVITE', inviteID);
  }
    ...
    if (id in this._db[tableName]) {
      ...
      for (const [fkey, tblPri, _] of fkeys) { // eslint-disable-line
        ...
        if (!(info[fkey] in this._db[tblPri])) {
          throw new DBError('Foreign key constraint error on ' +
            `\`${tableName}\`.\`${fkey}\``);
        }
      }
...
let inst = null;
module.exports = {
  DBError, Database, DatabaseTransaction, ProjectStatus,
  getInst: () => inst,
  setInst: (i) => inst = i,
};
