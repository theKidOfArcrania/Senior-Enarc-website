...
class Database extends Object {
  ...
  async insertFacultyInfo(uid, userinfo) {
    this._db.FACULTY[uid] = copyAttribs({}, userinfo, {tid: 0});
    this._db.FACULTY_OR_TEAM[userinfo.tid] = {isRegTeam: false};
    this._db.CHOICE[userinfo.tid] = Array(6).fill(null);
  }
  ...
  async insertTeamInfo(tid, teamInfo) {
    ...
    this._db.TEAM[tid] = copyAttribs({}, teamInfo, {assignedProj: '',
      budget: 1337, leader: '', tid: null, password: ''});
    this._db.CHOICE[tid] = (teamInfo.choices || [])
        .concat(range(6).fill(null)).slice(0, 6);
  }
  ...
  async loadUserInfo(uid) {
    return await this._loadEntity(uid, 'USER',
  ...
  async loadProjectInfo(pid) {
    return await this._loadEntity(pid, 'PROJECT',
  ...
  async loadEmployeeInfo(uid) {
    return await this._loadEntity(uid, 'EMPLOYEE',
  ...
  async loadUTDInfo(uid) {
    return await this._loadEntity(uid, 'UTD_PERSONNEL',
  ...
  async loadStudentInfo(uid) {
    return await this._loadEntity(uid, 'STUDENT',
  ...
  async loadFacultyInfo(uid) {
    return await this._loadEntity(uid, 'FACULTY',
