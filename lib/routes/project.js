...
async function loadUser(tr, uid) {
  ...
  await u.reload(tr);
  return utils.copyAttribs({}, u, {'uid': null, 'fname': null, 'lname': null,
    'email': null});
}
...
async function loadProject(tr, u, pid) {
  ...
  try {
    p = await db.loadProjectInfo(tid);
  } catch (e) {
    return null;
  }

  const access = projectRestrictionLevel(u, p);
...
r.get('/project', util.login, asyncHan(async (req, res) => {
  ...
  // TODO:
  getInst().doRTransaction(async (tr) => {
    const pids = await partOfProjs(tr, u);
    // Find the first project to view...
    for (const pid of pids) {
      const p = loadProject(tr, u, pid);
      if (p) {
        res.json(msg.success('Success', p));
        return;
      }
    }
    res.json(msg.fail('You have no projects to view.'));
  });
}));

r.get('/project/submit', util.login, util.employee,
    asyncHan(async (req, res) => {
      const e = req.employee;
      getInst().doTransaction(async (tr) => {
        const comp = tr.loadCompanyInfo(e.worksAt);
        if (comp.manager !== e.uid) {
          // TODO: should this be the case
          res.json(msg.fail('You are not allowed to submit project ' +
        ...
        proj.visible = false;
        proj.advisor = null;
        proj.cName = e.worksAt;

        try {
          tr.insertProjectInfo(proj.projID, proj);
          res.json(msg.success('Project successfully added. Your proposal ' +
            'will now be promptly reviewed by a staff or admin.'));
          return true;
        } catch (e) {
          res.json(msg.fail('Unable to add the project.', 'internal'));
          return false;
        }
      });
    }));
...
r.get('/project/list', util.login, asyncHan(async (req, res) => {
  ...
  await getInst().doRTransaction(async (tr) => {
    const projs = (await Promise.all((await db.findAllProjects())
