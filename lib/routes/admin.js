...
function restAPIFor(entity, opts = {}) {
  ...
  admin.post(`/${entity}`, asyncHan(async (req, res) => {
    const data = opts.processInput(req.bodySan);
    const success = await getInst().doTransaction(async (tr) => {
      const id = tr.findUniqueID(entity);
  ...
  admin.get(`/${entity2}`, asyncHan(async (req, res) => {
    ...
    if (ent === null) {
      ...
      res.json(msg.success('Success!', ent));
    }
  }));
  ...
  admin.post(`/${entity2}/list`, asyncHan(async (req, res) => {
    ...
    const lst = await getInst().doRTransaction(async (tr) => {
      return await Promise.all(ids.map(opts.loadFn.bind(null, tr)));
    });
    res.json(msg.success('Success', lst.map(opts.processOutput)));
  }));
...
// TODO: hash team passwords
restAPIFor('Team', {idField: 'tid', parseID: parseInt2});
// TODO: user subclasses
// TODO: hash employee passwords
// TODO: convert user types (or change user types to strings)
restAPIFor('User', {idField: 'userId', parseID: parseInt2});
