...
r.post('/team/join', util.login, util.student, asyncHan(async (req, res) => {
  ...
  const success = await getInst().doTransaction(async (tr) => {
    const tid = req.bodySan;
    try {
      await tr.loadTeamInfo(tid);
    } catch (e) {
      res.json(msg.fail('That team does not exist', 'badteam'));
      return false;
    }
  ...
  if (success) {
    res.json(msg.success('You have successfully joined the team!'));
  }
}));
...
r.post('/team/leave', util.student, asyncHan(async (req, res) => {
  const s = req.student;
  const success = await getInst().doTransaction(async (tr) =>
    await tr.alterStudentInfo(s.uid, {memberOf: null}));
  if (success) {
    res.json(msg.success('You have successfully left the team!'));
  }
}));
